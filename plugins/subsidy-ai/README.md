...
EDINET連携は親会社をゼロから推測するためのものではありません。利用者に親会社候補を確認したうえで `verify_corporate_relationship` を使用します。確認した提出書類に対象会社名がない場合も、資本関係がないとは断定しません。EDINET連携にはCloudflare Secretの `EDINET_API_KEY` が必要です。全国の企業関係データは保存せず、検証時に公式の最新版を照会します。
